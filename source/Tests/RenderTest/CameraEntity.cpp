...
CameraEntity::CameraEntity() : Entity()
{
	...
	const float cameraHeight = 1080.0f / 2;
	const float cameraWidth = 1920.0f / 2;
	...
	m_pCameraComponent = reinterpret_cast<EG::CameraComponent*>(EG::ComponentManager::GetInstance().CreateComponent<EG::CameraComponent>(
		...
		EG::Vector2D(windowWidth, windowHeight), EG::Vector2D(0.0f, 0.0f), EG::Vector2D(windowWidth, windowHeight)));
